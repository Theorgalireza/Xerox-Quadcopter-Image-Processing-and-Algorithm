...
class WindowDetector:
    def __init__(self):
        ...
        self.search_yaw_speed = 40  # Smooth rotation speed
        cv2.namedWindow("HSV")
        # HSV1
        ...
        cv2.createTrackbar("LH1", "HSV1", 179, 179, lambda x: None)
        cv2.createTrackbar("LS1", "HSV1", 89, 255, lambda x: None)
        cv2.createTrackbar("LV1", "HSV1", 80, 255, lambda x: None)
        cv2.createTrackbar("UH1", "HSV1", 178, 179, lambda x: None)
        cv2.createTrackbar("US1", "HSV1", 255, 255, lambda x: None)
        cv2.createTrackbar("UV1", "HSV1", 255, 255, lambda x: None)

        # HSV2
        cv2.namedWindow("HSV2")
        cv2.resizeWindow("HSV2", 300, 300)
        cv2.createTrackbar("LH2", "HSV2", 179, 179, lambda x: None)
        cv2.createTrackbar("LS2", "HSV2", 16, 255, lambda x: None)
        cv2.createTrackbar("LV2", "HSV2", 46, 255, lambda x: None)
        cv2.createTrackbar("UH2", "HSV2", 136, 179, lambda x: None)
        cv2.createTrackbar("US2", "HSV2", 255, 255, lambda x: None)
        cv2.createTrackbar("UV2", "HSV2", 114, 255, lambda x: None)

        # HSV3
        cv2.namedWindow("HSV3")
        cv2.resizeWindow("HSV3", 300, 300)
        cv2.createTrackbar("LH3", "HSV3", 179, 179, lambda x: None)
        cv2.createTrackbar("LS3", "HSV3", 162, 255, lambda x: None)
        cv2.createTrackbar("LV3", "HSV3", 48, 255, lambda x: None)
    ...
    def perform_search(self, tello):
        ...
        if not hasattr(self, "first_search_done"):
            ...
            self.search_yaw_angle = 0
            self.search_direction = 1  # First Turn Right
            self.throttle_direction = 1
            self.throttle_magnitude = 10  # Initial value throttle
            self.throttle_time_accumulator = 0  # Accumulated time for throttle
            self.throttle_speed = self.search_yaw_speed / 2  # Half yaw speed
        ...
        if not self.first_search_done:
            ...
            # Throttle movement (at half yaw speed)
            self.throttle_time_accumulator += dt

            # Let's assume that each yaw cycle (complete rotation) has a specific duration, for example based on angle and speed:
            yaw_cycle_duration = (140 * 2) / self.search_yaw_speed  # Time for a complete yaw cycle (back and forth)
            throttle_toggle_interval = yaw_cycle_duration * 2  # Throttle changes direction every two yaw cycles.
            ...
            if self.throttle_time_accumulator >= throttle_toggle_interval:
                ...
                # Increase throttle range when changing direction
    ...
    def process(self, frame, pid_yaw, tello, pid_throttle, pid_roll, pid_forward):
        ...
        if best_window is not None:
            self.no_detection_start_time = None
            self.searching = False
            self.search_yaw_angle = 0  # Reset Angle
            self.search_direction = 1
            ...
            if M["m00"] != 0:
                ...
                # Thresholds
                error_x_threshold = 15
                error_y_threshold = 10
                error_roll_threshold = 7
                error_z_threshold = 10000
                ...
                if pid_roll is not None and abs(error_roll) > error_roll_threshold:
                ...
                target_area = 130000
                area_error = target_area - best_area
                if abs(area_error) < error_z_threshold:
                    forward_backward_speed = 0
                else:
                    forward_backward_speed = int(np.clip(pid_forward.update(area_error), -30, 30))
                ...
                if (abs(error_roll) < error_roll_threshold and
                        ...
                        abs(area_error) < error_z_threshold and
                        self.last_confidence == 1.0):
                    tello.send_rc_control(0, 0, 0, 0)
                    time.sleep(1)
        ...
        if best_window is None:
            ...
            elif time.time() - self.no_detection_start_time > 4:  # If 3 Seconds passed
                self.searching = True
...
class DroneController:
    def __init__(self):
        ...
        time.sleep(0.5)  # A short pause for the command to be executed correctly.
        ...
        self.pid_forward = PID(Kp=0.001, Ki=0.0001, Kd=0.005)  # Initial values, adjustable by testing
    ...
    def run(self):
        try:
            while True:
                ...
                output = self.detector.process(frame, self.pid_yaw, self.tello, self.pid_throttle, self.pid_roll, self.pid_forward)
